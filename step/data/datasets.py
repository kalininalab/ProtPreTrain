...
# Remove wandb import if not needed
...
class FoldCompDataset(Dataset):
    """Save FoldSeekDB as a PyTorch Geometric dataset, using WebDataset."""
    ...
    def __init__(
        ...
        num_workers: int = 8,
        tar_shard_size: int = 100,  # Number of samples per shard
    ) -> None:
        self.db_name = db_name
        self.num_workers = num_workers
        self.tar_shard_size = tar_shard_size
        super().__init__(
            root=f"data/{db_name}", transform=transform, pre_transform=pre_transform
        )
    ...
    def processed_file_names(self):
        # WebDataset uses tar files as processed output
        # FIXME currently not checking for all files!
        return [f"graphs-{shard_id}-0.tar" for shard_id in range(self.num_workers)]
    ...
    def process_chunk(self, shard_id: int, start_num: int, end_num: int):
        """Process a single chunk of the database and save as tar shards."""
        cpu_count = multiprocessing.cpu_count()
        torch.set_num_threads(floor(cpu_count / self.num_workers))
        with foldcomp.open(self.raw_paths[0]) as db:
            sink = wds.TarWriter(
                f"{self.processed_dir}/graphs-{shard_id}.tar", compress=False
            )
            for idx in range(start_num, end_num):
                ...
                sink.write({"__key__": f"{idx}", "graph.pkl": pickle.dumps(data)})
            sink.close()  # Close the last shard

    def process(self) -> None:
        """Process the whole dataset and save as WebDataset tar shards."""
        print("Processing chunks in parallel...")
        with foldcomp.open(self.raw_paths[0]) as db:
            num_entries = len(db)
        tasks = [
            (i, x, x + self.tar_shard_size)
            for i, x in enumerate(range(0, num_entries, self.tar_shard_size))
        ]
        tasks[-1] = (tasks[-1][0], tasks[-1][1], num_entries)
        print(tasks)
        Parallel(n_jobs=self.num_workers)(
            delayed(self.process_chunk)(*args)
            for args in tasks
        )
        # print("Cleaning up...")
        # self.clean()  # Remove original FoldComp files if needed

    def clean(self):
        """Remove the original FoldComp files."""
        for filename in self.raw_file_names:
            filepath = os.path.join(self.raw_dir, filename)
            if os.path.exists(filepath):
                os.remove(filepath)

    def to_dataset(self) -> wds.WebDataset:
        """Convert the processed dataset to a WebDataset."""
        return wds.WebDataset(f"{self.processed_dir}/graphs-{{0..{}}}.tar")

    def len(self) -> int:
        """Not easily implemented for WebDataset. It requires iterating over all shards."""
        with foldcomp.open(self.raw_paths[0]) as db:
            return len(db)
...
class DownstreamDataset(InMemoryDataset):
    ...
    def __init__(
        self, split: str, *, transform=None, pre_transform=None, pre_filter=None
    ):
    ...
    def process(self):
        ...
        for _split, idx in self.splits.items():
...
class FluorescenceDataset(DownstreamDataset):
    ...
    def _prepare_data(self, df: pd.DataFrame) -> List[Data]:
        ...
        df["edits"] = df.apply(
            lambda row: compute_edits(orig_sequence, row["primary"]), axis=1
        )
        df["graph"] = df.apply(
            lambda row: apply_edits(orig_graph, row["edits"]), axis=1
        )
        data_list = [
            Data(
