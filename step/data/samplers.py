...
class DynamicBatchSampler(torch.utils.data.sampler.BatchSampler):
    ...
    def __init__(
        ...
        if max_num <= 0:
            raise ValueError(f"`max_num` should be a positive integer value " f"(got {max_num})")
        if mode not in ["node", "edge"]:
            raise ValueError(f"`mode` choice should be either " f"'node' or 'edge' (got '{mode}')")
